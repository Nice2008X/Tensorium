...
export function ModelLoader({ status, error, progress, onLoad, onLoadLocal, excludeRepo, embedded }: Props) {
  ...
  return (
    <div className={"model-loader" + (embedded ? " embedded" : "")}>
      {!embedded && (
        <>
          <div className="model-loader-title-row">
            <div className="model-loader-title">{t("loader.title")}</div>
            <a
              className="model-loader-github-link"
              href="https://github.com/Nice2008X/Tensorium"
              target="_blank"
              rel="noopener noreferrer"
              title={t("loader.viewOnGithub")}
              aria-label={t("loader.viewOnGithub")}
            >
